...
export default function App() {
  ...
  const filtered = useMemo(() => {
    ...
    return shipments.filter((item) => {
      ...
      return statusMatch && (!query || values.some((value) => value.toLowerCase().includes(query)));
    });
  ...
  const locateTruck = async () => {
    ...
    try {
      ...
      Alert.alert('موقع الشاحنة', `خط العرض: ${current.coords.latitude.toFixed(5)}\nخط الطول: ${current.coords.longitude.toFixed(5)}`);
    } catch {
  ...
  const renderShipment = ({ item }) => {
    ...
    return <View style={styles.card}>
      ...
      <View style={styles.actions}><TouchableOpacity style={styles.primary} onPress={() => updateStatus(item.id)}><Text style={styles.buttonText}>تحديث الحالة</Text></TouchableOpacity><TouchableOpacity style={styles.delete} onPress={() => removeShipment(item.id)}><Text style={styles.deleteText}>حذف</Text></TouchableOpacity></View>
    </View>;
  };

  return <SafeAreaView style={styles.container}>
    <StatusBar barStyle="light-content" backgroundColor="#123047" />
    <View style={styles.header}><View><Text style={styles.headerTitle}>نقل كاميون 🚚</Text><Text style={styles.headerSubtitle}>إدارة الشاحنات والرحلات</Text></View><TouchableOpacity style={styles.add} onPress={() => setShowForm(true)}><Text style={styles.addText}>＋ رحلة</Text></TouchableOpacity></View>
